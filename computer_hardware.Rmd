...
hardware_df <- original_data %>% 
  select(MYCT, MMIN, MMAX, CACH, CHMIN, CHMAX, PRP, ERP)
```
...
```{r}
linearModule <-lm(PRP~MYCT+MMIN+MMAX+CACH+CHMIN+CHMAX,data=hardware_df)
...
SSR <- ((linearModule$fitted.values - mean(hardware_df$PRP)) ^ 2) %>% sum()
...
Trực quan mô hình
```{r}
ggplot(data = hardware_df, aes(x = PRP, y = linearModule$fitted.values)) +
  geom_point() +
